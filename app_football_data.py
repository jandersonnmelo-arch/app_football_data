    ...
    if not jogos:
        ...
        for j in jogos:
            try:
                ...
                st.write(f"📊 Probabilidades: Casa {dc['pV']}% | Empate {round((dc['pE']+df['pE'])/2,1)}% | Fora {df['pD']}%")
                st.write(f"🎯 Chutes ao Gol: {total_chute} | Finalizações: {total_fin} | Faltas: {total_fal}")
                st.divider()
            except Exception as e:
